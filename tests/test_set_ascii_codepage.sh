#!/bin/bash
#
# Windows Shortcut File (EVTX) format library set ASCII codepage testing script
#
...
EVTX_TEST_SET_ASCII_CODEPAGE="evtx_test_set_ascii_codepage";

if ! test -x ${EVTX_TEST_SET_ASCII_CODEPAGE};
then
	EVTX_TEST_SET_ASCII_CODEPAGE="evtx_test_set_ascii_codepage.exe";
fi

if ! test -x ${EVTX_TEST_SET_ASCII_CODEPAGE};
then
	echo "Missing executable: ${EVTX_TEST_SET_ASCII_CODEPAGE}";
...
do
	./${EVTX_TEST_SET_ASCII_CODEPAGE} ${CODEPAGE};
