             ...
             uint32_t column_type )
{
	switch( column_type )
	{
		...
		case LIBEVTX_COLUMN_TYPE_BINARY_DATA:
			return( "Binary data" );

		case LIBEVTX_COLUMN_TYPE_TEXT:
			return( "Text" );

		case LIBEVTX_COLUMN_TYPE_LARGE_BINARY_DATA:
			return( "Large binary data" );

		case LIBEVTX_COLUMN_TYPE_LARGE_TEXT:
			return( "Large text" );

		case LIBEVTX_COLUMN_TYPE_SUPER_LARGE_VALUE:
     ...
     liberror_error_t **error )
{
	...
	if( libevtx_file_get_creation_format_version(
	     info_handle->input_file,
	     &format_version,
	     &format_revision,
	     error ) != 1 )
...
/* TODO refactor */
	     ...
	     table_iterator++ )
	{
		...
		if( libevtx_table_get_identifier(
		     table,
		     &table_identifier,
		     error ) != 1 )
		{
...
#if defined( LIBCSTRING_HAVE_WIDE_SYSTEM_CHARACTER )
		result = libevtx_table_get_utf16_template_name_size(
		          table,
		          &value_string_size,
		          error );
#else
		result = libevtx_table_get_utf8_template_name_size(
		          table,
		          &value_string_size,
		          error );
#endif
...
#if defined( LIBCSTRING_HAVE_WIDE_SYSTEM_CHARACTER )
			result = libevtx_table_get_utf16_template_name(
			          table,
			          (uint16_t *) value_string,
			          value_string_size,
			          error );
#else
			result = libevtx_table_get_utf8_template_name(
			          table,
			          (uint8_t *) value_string,
			          value_string_size,
			          error );
		...
		if( libevtx_table_get_number_of_columns(
		     table,
		     &number_of_columns,
		     LIBEVTX_GET_COLUMN_FLAG_IGNORE_TEMPLATE_TABLE,
		     error ) != 1 )
		{
			liberror_error_set(
			 error,
		     ...
		     column_iterator++ )
		{
			...
			if( libevtx_column_get_identifier(
			     column,
			     &column_identifier,
			     error ) != 1 )
			{
		     ...
		     index_iterator++ )
		{
			...
			if( libevtx_index_get_identifier(
			     index,
			     &index_identifier,
			     error ) != 1 )
			{
