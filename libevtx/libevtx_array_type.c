...
{
	static char *function = "libevtx_array_initialize";
...
int libevtx_array_free(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_free";
	int result            = 1;
...
int libevtx_array_empty(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_empty";
...
int libevtx_array_clear(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_clear";
...
int libevtx_array_clone(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_clone";
	int entry_iterator    = 0;
     ...
     liberror_error_t **error )
{
	...
	static char *function = "libevtx_array_resize";
	size_t entries_size   = 0;
	int entry_iterator    = 0;
	int result            = 1;
...
int libevtx_array_get_number_of_entries(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_get_number_of_entries";
...
int libevtx_array_get_entry_by_index(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_get_entry_by_index";
...
int libevtx_array_set_entry_by_index(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_set_entry_by_index";
...
int libevtx_array_append_entry(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_array_append_entry";
 ...
 * The entry_compare_function should return LIBEVTX_ARRAY_COMPARE_LESS,
 * LIBEVTX_ARRAY_COMPARE_EQUAL, LIBEVTX_ARRAY_COMPARE_GREATER if successful or -1 on error
 *
 * Duplicate entries are allowed by default and inserted after the last duplicate entry.
 * Only allowing unique entries can be enforced by setting the flag LIBEVTX_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES
 *
...
int libevtx_array_insert_entry(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_tree_node_insert_node";
	int entry_iterator    = 0;
	int result            = -1;
	...
	if( ( insert_flags & ~( LIBEVTX_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) ) != 0 )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( array->entries != NULL )
	{
		     ...
		     entry_iterator++ )
		{
			...
			else if( result == LIBEVTX_ARRAY_COMPARE_EQUAL )
			{
				if( ( insert_flags & LIBEVTX_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) != 0 )
				{
					return( 0 );
			...
			else if( result == LIBEVTX_ARRAY_COMPARE_LESS )
			{
				break;
			}
			else if( result != LIBEVTX_ARRAY_COMPARE_GREATER )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( ( array->entries != NULL )
	 && ( result == LIBEVTX_ARRAY_COMPARE_LESS ) )
	{
