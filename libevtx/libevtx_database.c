...
#include "libevtx_database.h"
#include "libevtx_page_tree.h"
...
{
	static char *function = "libevtx_database_initialize";
	...
	if( *database == NULL )
	{
		*database = memory_allocate_structure(
		...
		if( memory_set(
		     ( *database ),
		     0,
		     sizeof( libevtx_database_t ) ) == NULL )
		{
			liberror_error_set(
			 error,
			 LIBERROR_ERROR_DOMAIN_MEMORY,
...
int libevtx_database_free(
     ...
     liberror_error_t **error )
{
	static char *function = "libevtx_database_free";
...
int libevtx_database_read(
     ...
     liberror_error_t **error )
{
	libevtx_page_tree_t *database_page_tree        = NULL;
	libevtx_values_tree_value_t *values_tree_value = NULL;
	...
	uint8_t *data                                   = NULL;
	static char *function                           = "libevtx_database_read";
	...
	if( libevtx_page_tree_initialize(
	     &database_page_tree,
	     io_handle,
	     pages_vector,
	     pages_cache,
	     LIBEVTX_FDP_OBJECT_IDENTIFIER_DATABASE,
	     NULL,
	     NULL,
	     error ) != 1 )
	{
	...
	if( libfdata_tree_initialize(
	     ...
	     &libevtx_page_tree_free,
	     NULL,
	     &libevtx_page_tree_read_node_value,
	     &libevtx_page_tree_read_sub_nodes,
	...
	if( libfdata_cache_initialize(
	     ...
	     LIBEVTX_MAXIMUM_CACHE_ENTRIES_TREE_VALUES,
	     error ) != 1 )
	{
		liberror_error_set(
		 error,
	...
	node_data_offset  = LIBEVTX_PAGE_NUMBER_DATABASE - 1;
	     ...
	     leaf_node_index++ )
	{
		...
		if( libevtx_values_tree_value_read_data(
		     values_tree_value,
		     file_io_handle,
		     io_handle,
		     pages_vector,
